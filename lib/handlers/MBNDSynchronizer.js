  ...
  fullSync = async () => {
    ...
    let musicBeeCollection = await csv2json({
      ...
      colParser: {
        ...
        "Last Played": item => (dayjs(item, 'DD/MM/YYYY HH:mm').isValid() ? dayjs(item, 'DD/MM/YYYY HH:mm').utc() : null),
    ...
    let tracksAnnotations = await Promise.all(
      musicBeeCollection.map(track =>
        limit(async () => {
          ...
          const filePathEnd = `${dbFolderSeparator}${
            track.folder
          }${dbFolderSeparator}${track.filename}`;

          const foundTrack = await Track.findOne({
            // unicode character in title field can cause issues, specifically: https://www.compart.com/en/unicode/U+0007
            where: {
              path: {
          ...
          if (track.lastPlayed > annotation.play_date || !annotation.play_date) { // can't compare something > null
